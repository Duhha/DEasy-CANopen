...
bool Easy_CANopen_Thread_Listen_Messages(CANopen *canopen) {
	...
	if (is_new_message) {
		...
		case FUNCTION_CODE_SDO_TRANSMIT:
			CANopen_Client_SDO_Receive_Response(canopen, node_ID, data);//CANopen_Server_SDO_Receive_Request(canopen, node_ID, data); /* Server -> Client */
			break;
		case FUNCTION_CODE_SDO_RECEIVE:
			CANopen_Server_SDO_Receive_Request(canopen, node_ID, data);//CANopen_Client_SDO_Receive_Response(canopen, node_ID, data); /* Client -> Server */
			break;
